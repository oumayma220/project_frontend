...
export class TestdragComponent {
  fields: Field[] = [];
  generatedSchema: any = {};
  rootType: 'object' | 'array' = 'object'; // Nouveau paramètre pour le type racine
  ...
  generateSchema() {
    if (this.rootType === 'object') {
      ...
      // Pour une racine de type array, on crée un schéma d'array
      const itemSchema = this.buildSchema(this.fields);
  ...
  copyToClipboard() {
    ...
    navigator.clipboard.writeText(jsonString)
      .then(() => {
        this.snackBar.open('JSON copié dans le presse-papiers !', 'OK', {
          duration: 3000,
